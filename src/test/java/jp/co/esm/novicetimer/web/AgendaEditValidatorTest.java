...
/**
 * アジェンダ編集フォームのバリデータクラスのテスト
 */
@RunWith(SpringRunner.class)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class AgendaEditValidatorTest {

    @Autowired
    private AgendaEditValidator validator;

    /**
     * AgendaEditValidatorを実行し、結果を返す。
     *
    ...
    public void アジェンダIDがないときエラーとなる() {
        ...
        BindingResult result = doValidate(null, Arrays.asList(new SubjectForm("タイトル１", "1", "ユーザ１")));
        assertThat(result.getFieldErrors(), hasSize(1));
        assertThat(result.getFieldErrors().get(0).getDefaultMessage(), equalTo("不正なリクエストです。"));

        result = doValidate("", null);
        assertThat(result.getFieldErrors(), hasSize(1));
        assertThat(result.getFieldErrors().get(0).getDefaultMessage(), equalTo("不正なリクエストです。"));

        // アジェンダIDはint型のため桁数は9桁までにしている
        result = createBindingResult();
        validator.validate(new AgendaForm("1000000000", null) ,result);
        assertThat(result.getFieldErrors(), hasSize(1));
        assertThat(result.getFieldErrors().get(0).getDefaultMessage(), equalTo("不正なリクエストです。"));

        // アジェンダIDがnullでないとき、それ以外のエラーは発生しない
        result = doValidate(" ", Collections.singletonList(
            new SubjectForm(
        ...
        result = doValidate("1", Arrays.asList(new SubjectForm("タイトル１", "1", "ユーザ１")));
    ...
    public void サブジェクトリストがnullまたは空のときエラーとなる() {
        ...
        BindingResult result = doValidate("1", Collections.emptyList());
        assertThat(result.getFieldErrors(), hasSize(1));
        ...
        result = doValidate("1", Collections.singletonList(new SubjectForm("", "", "")));
        assertThat(result.getFieldErrors(), hasSize(1));
        assertThat(result.getFieldErrors().get(0).getDefaultMessage(), equalTo("入力がありません。"));
        ...
        result = doValidate("1", Collections.singletonList(new SubjectForm("タイトル１", "1", "ユーザ１")));
        assertThat(result.getFieldErrors(), hasSize(0));
    }
    ...
    public void 全項目が値なしのサブジェクトはOKとなる() {
        ...
        BindingResult result = doValidate("1", Arrays.asList(
            new SubjectForm("タイトル１", "1", "ユーザ１"),
            new SubjectForm("", "", "")));
        assertThat(result.getFieldErrors(), hasSize(0));
